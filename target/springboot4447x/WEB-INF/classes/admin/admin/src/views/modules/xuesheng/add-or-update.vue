<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="出生年月" prop="chushengnianyue">
          <el-input v-model="ruleForm.chushengnianyue" 
              placeholder="出生年月" clearable  :readonly="ro.chushengnianyue"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="出生年月" prop="chushengnianyue">
              <el-input v-model="ruleForm.chushengnianyue" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="辅导员工号" prop="fudaoyuangonghao">
          <el-select  @change="fudaoyuangonghaoChange" v-model="ruleForm.fudaoyuangonghao" placeholder="请选择辅导员工号">
            <el-option
                v-for="(item,index) in fudaoyuangonghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.fudaoyuangonghao" label="辅导员工号" prop="fudaoyuangonghao">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="个人技能" prop="gerenjineng">
          <file-upload
          tip="点击上传个人技能"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.gerenjineng?ruleForm.gerenjineng:''"
          @change="gerenjinengUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.gerenjineng" label="个人技能" prop="gerenjineng">
            <el-button type="text" size="small" @click="download(ruleForm.gerenjineng)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="奖项证书" prop="jiangxiangzhengshu">
          <file-upload
          tip="点击上传奖项证书"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.jiangxiangzhengshu?ruleForm.jiangxiangzhengshu:''"
          @change="jiangxiangzhengshuUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.jiangxiangzhengshu" label="奖项证书" prop="jiangxiangzhengshu">
            <el-button type="text" size="small" @click="download(ruleForm.jiangxiangzhengshu)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.gerenjineng = true;
          ...
          if(o=='jiangxiangzhengshu'){
            this.ruleForm.jiangxiangzhengshu = obj[o];
	    this.ro.jiangxiangzhengshu = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/fudaoyuan/fudaoyuangonghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.fudaoyuangonghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    fudaoyuangonghaoChange () {
      this.$http({
        url: `follow/fudaoyuan/fudaoyuangonghao?columnValue=`+ this.ruleForm.fudaoyuangonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.fudaoyuanxingming){
                this.ruleForm.fudaoyuanxingming = data.data.fudaoyuanxingming
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    gerenjinengUploadChange(fileUrls) {
	this.ruleForm.gerenjineng = fileUrls;
			this.addEditUploadStyleChange()
    },
    jiangxiangzhengshuUploadChange(fileUrls) {
	this.ruleForm.jiangxiangzhengshu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
